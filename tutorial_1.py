...
class FittingObjects(Scene):
    def construct(self):
        ...
        circle.set_width(2).to_edge(DR, buff=0.8) # DR: Down Right

        triangle = Triangle(stroke_color=ORANGE, stroke_width=8,
                            fill_color=GREY)
...
class ValueTracking(Scene):
    def construct(self):
        ...
        r = ValueTracker(0.5)    # tracks radius the radius of the circle
        ...
        Polygon(circle.get_top(), circle.get_left(), circle.get_right(), 
                fill_color=GREEN_C))
