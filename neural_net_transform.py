from manim import *
from manim.utils.color import WHITE, BLACK, PURPLE, RED, GRAY
import numpy as np
import math
...
class LinearTransform(Scene):
    ...
    def linear_transfom(self):
        ...
        ).set_stroke(color=BLACK)

        ax.get_axis(0).numbers.set_color(BLACK).set_stroke(width=1)
        ax.get_axis(1).numbers.set_color(BLACK).set_stroke(width=1)

        ax_border = Rectangle(width=ax.x_length, height=ax.y_length).set_stroke(
            color=BLACK, width=4
        )

        grid = NumberPlane(ax, [-6, 6, 0.5], [-6, 6, 1], lattice_radius=0.04)
...
class TanhGraph(Scene):
    ...
    def build_tanh_graph(self):
        ...
        tanh_text = (
            MathTex(r"\tanh(x)=\frac{e^x - e^{-x}}{e^x + e^{-x}}", color=BLACK)
            .set_stroke(width=1)
            .scale(0.8)
            .move_to(ax.get_corner(UL))
        ...
        graph = ax.plot(math.tanh, color=PURPLE)
        labels = ax.get_axis_labels(x_label=Tex("x"), y_label=Tex("y"))

        t = ValueTracker(0)
        initial_point = [ax.c2p(t.get_value(), math.tanh(t.get_value()))]
        dot = Dot(point=initial_point, radius=0.04, color=BLACK)
    ...
    def show_squishing(self):
        ...
        fixed_grid.axes.set_color(BLACK).set_stroke(width=2)

        grid = NumberPlane(fixed_grid, [-3.5, 3.5, 0.5], [-2, 2, 0.5], lattice_radius=0.04)
...
class ReLUGraph(Scene):
    ...
    def build_relu_graph(self):
        ...
        relu_text = (
            MathTex(r"\text{relu}(x)=\text{max}(0, x)", color=BLACK)
            .set_stroke(width=1)
            .scale(0.8)
            .move_to(ax.get_corner(UL))
        ...
        graph = ax.plot(lambda x: max(0, x), color=PURPLE, use_smoothing=False)
        labels = ax.get_axis_labels(x_label=Tex("x"), y_label=Tex("y"))
...
class LeakyReLUGraph(Scene):
    ...
    def build_leaky_relu_graph(self):
        ...
        leaky_relu_text = (
            MathTex(r"\text{leaky\_relu}(x)=\text{max}(ax, x)", color=BLACK)
            .set_stroke(width=1)
            .scale(0.8)
            .move_to(ax.get_corner(UL))
            .shift(DR * 0.7 + RIGHT * 2)
        ...
        graph = ax.plot(
            lambda x: max(0.1 * x, x), color=PURPLE, use_smoothing=False
        )
        labels = ax.get_axis_labels(x_label=Tex("x"), y_label=Tex("y"))
...
class AffineNonLinearTransform(Scene):
    ...
    def affine_nonlinear_transform(self):
        ...
        dot = Dot(point=initial_point, radius=0.08, color=PURPLE)
        dot.add_updater(
        ...
        tanh_text = (
            MathTex(r"\tanh(Wx + b)", color=WHITE)
            .set_stroke(width=1)
            .move_to(ax.get_corner(UL))
            .shift(RIGHT * 1.8 + DOWN * 0.5)
        )
